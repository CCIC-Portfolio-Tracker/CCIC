...
async function importOneYearTWR() {
    // Fetch all daily values and all cash flows for one year
    const values = await db.execute("SELECT tot_value, value_date FROM value_table WHERE value_date >= date('now', '-1 year') ORDER BY value_date ASC ");
...
export default importOneYearTWR;